...
1. :meth:`~object.__aiter__` is called to retrieve the asynchronous
   generator iterator.
2. Calls to :meth:`~agen.asend`, :meth:`~agen.athrow`, and :meth:`~agen.aclose` are
   delegated instead of the usual :meth:`~gen.send`, :meth:`~gen.throw`, and
   :meth:`~gen.close` methods.
3. :exc:`StopAsyncIteration` is used to retrieve the return value instead of
   :exc:`StopIteration`.

``async yield from`` is only allowed in an asynchronous generator function;
using it elsewhere will raise a :exc:`SyntaxError`.

In an asynchronous generator, ``async yield from`` is broadly equivalent to:
...
But, like with ``yield from``, ``async yield from`` also provides the standard
subgenerator delegation behavior. This is outlined in :pep:`380`
and :ref:`the documentation <yieldexpr>`.
...
``async yield from`` formal semantics
-------------------------------------

.. code-block:: python

    _aiterator = await aiter(EXPR)
    try:
        _delegated_next = await anext(_aiterator)
    except AsyncStopIteration as _e:
        RESULT = _e.value
    else:
        while True:
            try:
                _received = yield _delegated_next
            except GeneratorExit:
                await _aiterator.aclose()
            except BaseException:
                _exc_info = sys.exc_info()
                try:
                    _delegated_next = await _aiterator.athrow(*_exc_info)
                except AsyncStopIteration as _stop_iteration:
                    RESULT = _stop_iteration.value
                    break
            else:
                try:
                    if _received is None:
                        ...
                        _delegated_next = _await aiterator.asend(_received)
                except StopAsyncIteration as _stop_iteration:
                    RESULT = _stop_iteration.value
                    break
